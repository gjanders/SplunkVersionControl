...
class PostToken(GeneratingCommand):
    ...
    def generate(self):
        ...
        """
        if self.url:
            urlregex = re.compile("^(?:https:\/\/)[\w.-]+(?:\.[\w\.-]+)+[\w\-\._~:/?#[\]@!\$&'\(\)\*\+,;=.]+$")
            if urlregex.match(self.url) or self.url=="https://localhost:8089/services/send":
                url = self.url
            else:
                yield {'result': 'Invalid URL passed in, url= must begin with https:// and would normally end in :8089, url provided %s' % (self.url) }
                return
        else:
...
dispatch(PostToken, sys.argv, sys.stdin, sys.stdout, __name__)
